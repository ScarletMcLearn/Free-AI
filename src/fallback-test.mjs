...
async function runLive() {
  ...
  const real = cfg.route.find((p) => p.credentialEnv && dotEnv[p.credentialEnv]);
  if (!real) {
    writeReport({
      time: new Date().toISOString(),
      mode: "live",
      pass: false,
      error: "No configured free provider credentials in .env"
    });
    return;
  }
  ...
  try {
    ...
    const text = await response.text();
    const pass = response.ok && events.some((e) => e.provider === "test-invalid" && e.result === "retryable") && events.some((e) => e.provider === real.id && e.result === "success");
    writeReport({ time: new Date().toISOString(), mode: "live", pass, firstAttempt: "test-invalid", fallbackProvider: real.id, fallbackModel: real.model, events, responseStatus: response.status, responseBody: text.slice(0, 1000) });
  } finally {
    router.close();
    failing.close();
  }
